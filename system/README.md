...
- データプレーンの実現方法
  - 要件や機能を満たすためにどのようなデータプレーンを実現したらよいかを考えます
    - データプレーンの実現方法はそのままコストやパフォーマンスに直結します
      - より少ない計算コスト、運用コストで、より多くのパフォーマンスを得られるようにしましょう
    - その後、そのデータプレーンを実現するためのコントロールプレーンを考えます
    - データプレーンを実際にどこまで実現できるかで、要件や機能が絞られたり、逆に拡張される場合もあります
    - データプレーンの目途が立たない場合は、要件・機能の具体化の前に、その検証を行う場合もあります
  - パフォーマンスの考慮
...
- システムに落とし込めるかを意識する
  - 最終的にはシステムに落とし組む必要があるので、システムに落とし込んだ場合もイメージして設計する
  - システムは役割ごとにサービスを分けて設計します
    - 各サービス間はインターフェイスを挟んで、それぞれのサービス内で独立してmockを使ってテストできるようにします
    - すべてのサービスが自分の管理下にあるわけではなく、それが他部署の管理下、あるいは社外のサービスを利用するケースもあります
      - いわゆるプラットフォーム系サービスをシステムに組み込むことはよくあることです
    - 各サービス間の循環参照は基本的にはしないほうがよいが、循環参照を許容して利用するケースもあります
      - プラットフォーム系サービスを自身のシステムに組こみたいが、そのプラットフォーム系サービスが自信のサービスを参照している場合があります
      - これはどのように循環参照しているかを把握できており、システム普及時に循環参照されていない抜け道がある場合に限ります
  - サービス内では、レイヤと機能でモジュールを分けて設計します
    - 各レイヤ間はインターフェイスを挟んで、それぞれのレイヤ内で独立してmockを使ってテストできるようにします
    - 各モジュールはそれぞれでunitテストできるようにします
    - 一つのサービス内での循環参照はもってのほかで、各レイヤの依存は一方向にします
- コントロールプレーンの運用性、メンテにおける障害耐性
  - コントローラはなるべく無停止メンテができるようにしましょう
  - SLAに影響せずにメンテをできるようにしましょう
    - メンテが稼働率などに影響しないようにSLAを設定しましょう
      - SL/SLI/SLO/SLAについて
        - SL(Service Level)とは、サービスの品質や状態を段階的に表したもの
        - SLI(Service Level Indicator)
          - サービスレベル目標を定量的に計測し目標値を満たしているかいないかを判断するための指標のこと
          - 可用性、エラー率、システムスループット、リクエストのレイテンシなど
            - サービスの特性に合わせた指標とします
        - SLO(Service Level Objective)
          - SLI であげた指標に対する目標数値のことで、「9」で表されることが多い（99.9%
          - SLI → SLO の順で決める
        - SLA(Service Level Agreement)とは、サービス提供者と利用者で合意されたサービス品質のこと
          - SLI であげた指標に対するサービスと利用者の合意
          - SLOは内々の目標だが、SLAは公の約束事なので、SLOの目標値よりもより現実的に守れる値とする(SLA < SLO)
      - 無理なSLAを設定して、運用がしずらくなるみたいな状況は避けましょう
...
このFDを意識して、どこに障害発生しても影響範囲を最小限に押さえられるよう設計することが重要である。

### FailerDomainの種類

![FailureDomainKind](./FailureDomainKind.dio.svg)

FDは、各レイヤーでなるべく合わせることが望ましいです。  
一般的にはAZ(AvailabilityZone)を定義して、データセンターの電源やネットワーク機器やサーバ配置などのFDをここに合わせことが多いです。  
FDはなるべくAZに合わせることが望ましいですが、FDを最小化するために無理に設計を変える必要はありません。   
システムによってはRegion単位で管理するものや、Global単位で管理するものもあるでしょう。  

FDは、データプレーンと、コントロールプレーンの両軸で考える必要があります。  
基本的な考え方は、AZが一つ落ちても問題ないようにシステムを設計することです。  
これに加えて、コントロールプレーンに関してはその影響範囲についても意識しましょう。  
耐障害性的にはAZが一つ落ちても大丈夫でも、そのコントローラがRegion単位で存在する場合、そのRegion単位で操作ができなくなる可能性もあります。  
基本的にコントロールプレーンの障害がデータプレーンの障害に直結するような設計はすべきではなく、コントロールプレーンの停止は許容されるべきです。  
とはいえ、頻繁にメンテなどでコントロールプレーンが止まるというのはユーザ側としては許容できるものではないので、基本的にオンラインでメンテが実施できる必要があります。  
もちろん、メンテの都合上どうしても止めないといけない場合もあるので、そこは許容してもらえるようSLAを設定しましょう。  
